...
n_p_histogram <- function(){
  val_n_p <- read.table("train_outputs/non_model/val_n_p.tsv", 
             header = T, sep = "\t", check.names = F)
  val_n_p$partition <- paste0("Validation\n(", round(100 * mean(val_n_p$n_p==0), 2), "% 0)")
  ...
  merged <- rbind(val_n_p, train_n_p)
  merged$partition <- factor(merged$partition, levels = c(paste0("Validation\n(", round(100 * mean(val_n_p$n_p==0), 2), "% 0)"), paste0("Train\n(", round(100*mean(train_n_p$n_p==0),2), "% 0)")))
...
density_interval_vs_nominal <- function(){
  ...
  for (i in 1:length(file_list)) {
    ...
    df_addition <- data.frame("capture_prop"=train_out$`mean capture proportion`,
          ...
          "partition" = ifelse(grepl("val", file_list[i]), "Validation", "Train"),
  ...
  density_interval_df$partition <- factor(density_interval_df$partition, levels=c("Train", "Validation"))
...
volcano_plot_mean <- function(){
  ...
  volcano_df$partition <- factor(volcano_df$partition, levels=c("Train", "Validation"))
...
auc_eval <- function(){
  ...
  auc_df$partition <- factor(auc_df$partition, levels=c("Train", "Validation"))
...
weighed_metrics <- function(){
  ...
  weighted_df$partition <- factor(weighted_df$partition, levels=c("Train", "Validation"))
...
mean_abs_err <- function(){
  ...
  for (i in 1:length(file_list)) {
    ...
    df_addition <- data.frame("mean_abs_err"=c(train_out$`mean abs err 1`, train_out$`mean abs err 2`[!is.na(train_out$`mean abs err 2`)]) / sds,
                              "partition" = ifelse(grepl("val", file_list[i]), "Val", "Train"),
                              "n_p" = ifelse(grepl("only_sim", file_list[i]), "first", "second"),
                              "model" = gsub("_.*", "", file_list[i]))
    weighted_df <- rbind(weighted_df, df_addition)
  ...
  weighted_df$partition <- factor(weighted_df$partition, levels=c("Train", "Val"))
  weighted_df$n_p <- factor(weighted_df$n_p)
  weighted_df$model <- case_when(weighted_df$model == "model1" ~ "Model 1",
                                 weighted_df$model == "model2" ~ "Model 2",
                                 weighted_df$model == "model3" ~ "Model 3",
