...
class Cont:
    ...
    def update(self, ID, IDs, ss, n_p):
        ...
        if n_p > 0:
            ...
            loss = 1 - np.sum(norm_c * std_norm_piece + 1/((n_p + 1) * np.sqrt(2 * math.pi)) * np.sum(std_phis, axis=1)) * self.delta / 101
            ...
            dldw0 = - np.sum(norm_d * np.multiply(dfydh, dhdw0)) * self.delta / 101
            dldw1 = - np.sum(norm_d * np.multiply(dfydh, dhdw1)) * self.delta / 101
            dldc = - np.sum(norm_d * np.multiply(dfydh, dhdc)) * self.delta / 101 + 2 * (self.c - eta) * self.beta
    ...
    def update(self, ID, IDs, ss, n_p):
        ...
        if n_p > 0:
            ...
            if y[2]:
                ...
                dldc2 = - np.sum(norm_d * np.multiply(dfydh2, dhdc2)) * y[0] * 0.2 / 101 * y[1] * 0.2 / 101 * (self.c_2 - eta_2) * self.beta_cont
